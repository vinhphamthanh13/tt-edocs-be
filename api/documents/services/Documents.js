...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Documents.associations
  ...
  fetch: (params) => {
    ...
    return Documents
      .findOne(_.pick(params, _.keys(Documents.schema.paths)))
      .populate(populate);
  },
  ...
  add: async (values) => {
    ...
    const entry = await Documents.create(data);

    // Create relational data and return the entry.
    return Documents.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Documents.associations.map(a => a.alias));
    const data = _.omit(values, Documents.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Documents.updateOne(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Documents.associations.map(async association => {
  ...
  /**
   * Promise to search a/an documents.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Documents.attributes).reduce((acc, curr) => {
      switch (Documents.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
